...
/dev/ttyUSB0 or 0.0.0.0:14550. Auto-detect serial if not given.')
    ...
    try:
        ...
        def erase_last_n_chars(N):
            if N == 0: return
            # Move cursor back N positions, write N spaces to erase, then move cursor back again
            sys.stdout.write('\b' * N + ' ' * N + '\b' * N)

        def get_visual_width(ch):
            """Get the visual width of a character on the terminal"""
            if ch == '\t':
                return 4  # tabs typically display as 4 spaces
            else:
                return 1
        ...
        while quit_time is None or quit_time > timer():
            while True:
                ...
                if ch == '\n':
                    ...
                    cur_line = ''
                elif ord(ch) == 8 or ord(ch) == 127: # backspace (ASCII 8) or DEL (ASCII 127)
                    if len(cur_line) > 0:
                        last_char = cur_line[-1]
                        visual_width = get_visual_width(last_char)
                        erase_last_n_chars(visual_width)
                        cur_line = cur_line[:-1]
                    sys.stdout.flush()
                elif ord(ch) == 9: # tab - send to remote shell for autocomplete
                    # Just send the tab character to remote
                    # Note: for autocomplete to work properly, the remote shell would need
                    # to know what we've typed so far, but since we buffer locally, 
                    # we'll just ignore tab to prevent display issues
                    pass
                elif ord(ch) == 27 :
